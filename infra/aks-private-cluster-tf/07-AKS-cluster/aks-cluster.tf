...
resource "azurerm_role_assignment" "appdevs_user" {
  ...
  # principal_id         = data.terraform_remote_state.aad.outputs.appdev_object_id # N/A because the AAD groups TF deployment fails 
  principal_id         = var.appdev_object_id 
}
...
resource "azurerm_role_assignment" "aksops_admin" {
  ...
  # principal_id         = data.terraform_remote_state.aad.outputs.aksops_object_id # N/A because the AAD groups TF deployment fails
  principal_id         = var.aksops_object_id
}
